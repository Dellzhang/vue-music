<template>
  <div class="content">
    ...
    <v-touch @swipeleft="swiperleft" @swiperight="swiperright" class="wrapper">
      <div>
        <div :id=" state == 0 ? '': 'stateLeft'" :class=" this.$store.state.show? 'imgWrapper ami': 'imgWrapper' ">
          <img :src="imgs" alt="">
        </div>
        <div :id="state == 1 ? 'StateRight': ''" class="LyricsList" ref="lyricList">
          <div class="box" :style="{transform: `translate(0px, ${-num*30}px) translateZ(0px)`}">
    ...
    <div class="bottom">
      <div class="prage">
        <p>{{playTime}}</p>
        <van-progress :percentage.sync="percentage" track-color="rgba(0,0,0,0.3)" color="#ffcd32" :show-pivot= "pivot"/>
        <p>{{times}}</p>
      </div>
